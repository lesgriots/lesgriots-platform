...
// Contenu interne du header (logo + menus + bouton), rendu 2 fois : une couche
// sombre de base et une couche blanche découpée au bord de la vidéo.
  ...
  return (
    <>
      ...
      <header className="lg__header">
        {/* Couche SOMBRE (de base) + couche BLANCHE superposée, découpée au ras
            du bord de la vidéo (clip-path piloté en JS via --vid-top/--vid-bottom).
            Résultat : le texte devient blanc pixel par pixel là où la vidéo passe
            derrière lui, transition vraiment progressive. */}
        <div className="lg__header__layer">
          <HeaderInner
        ...
        <div className="lg__header__layer lg__header__layer--white" aria-hidden="true">
          <HeaderInner utilityLinks={utilityLinks} navLinks={navLinks} onMenu={() => {}} />
        </div>
      </header>
...
function GriotRing() {
  return (
    <span className="lg__hero__griot__wrap">
      <svg
      ...
      >
        {/* Mot-marque officiel vectorisé (branding/logo-lagriotheque-wordmark-*.svg).
            Tracés Geist sans bold + ™ — rendu identique partout, indépendant des
            polices chargées. currentColor → s'adapte clair/sombre. */}
        <g
...
function HoverBg({ src }) {
  ...
  return cur.img ? (
    ...
    <div
      ...
      style={{ left: cur.x + "px", top: cur.y + "px" }}
    >
      <span>{trainerInitials(cur.name)}</span>
    </div>
  );
}
...
function EventCard({ e }) {
  // Liste en lignes (réf. fournie) : titre + méta à gauche, lien à droite,
  // filets horizontaux entre les lignes. Toute la ligne mène à la fiche.
            ...
            <div className="lg__approche__about__col">
              ...
              <div className="lg__contact__eco">
                ...
                <a className="lg__contact__eco__card" href="https://lesgriotsxstudio.com" target="_blank" rel="noopener noreferrer">
                  ...
                  <span className="lg__contact__eco__desc">Le studio : direction artistique et production audiovisuelle pour artistes et marques.</span>
                </a>
              </div>
            </div>
          )}
...
function App() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // blanche → le texte devient blanc progressivement, au ras de la vidéo.
      const headerEl = document.querySelector(".lg__header");
      if (headerEl) {
