/**
 * /api/sessions/:id/emargement — le emargement de la session, en PDF.
 *
 ...
 * emargement d'une génération précédente, sans que rien ne le signale.
 *
...
async function _GET(req, { params }) {
  try {
    ...
    const { pdf, nom } = await rendreDocumentSession(getDb(), 'emargement', id);
